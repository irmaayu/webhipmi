...
class Pasar_upi extends CI_Controller {
	...
	public function index($order = NULL,$offset =0)
	{
		...
		$data['menu_pasar_upi'] = 1;
		$data['slider'] = 1;
		$data['isi'] = 'konten/pasar_upi';
		$data['title'] = 'Pasar UPI';

		if ($order != NULL) {
			 $data['list_produk'] = $this->ProdukModel->get_all_order($order);
			
		}else {
			$data['list_produk'] = $this->ProdukModel->get_all();
		}
		$data['list_slider4']= $this->SliderModel->get_slider(4,$offset);
		$data['list_slider5']= $this->SliderModel->get_slider(5,$offset);
		$data['list_slider6']= $this->SliderModel->get_slider(6,$offset);


		$data['pria'] = $this->ProdukModel->get_kategori(1);
		$data['wanita'] = $this->ProdukModel->get_kategori(2);
		$data['umum'] = $this->ProdukModel->get_kategori(3);
